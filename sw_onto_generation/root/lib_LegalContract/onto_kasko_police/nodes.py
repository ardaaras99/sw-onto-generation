...
class KaskoPolice(BaseNode):
    """Kasko sigortası poliçesinin temel bilgilerini tanımlar. Poliçe numarası, türü, süresi, tanzim tarihi gibi ana bilgileri içerir."""
...
class Arac(BaseNode):
    ...
    motor_no: str | None = Field(
        default=None,
        description="Araç motor numarası (örn. 64286741764928)",
        config=NodeFieldConfig(index_type=NebulaIndexType.EXACT),
    )
    sasi_no: str | None = Field(
        default=None,
        description="Araç şasi numarası (örn. WDD2421231A239929)",
        config=NodeFieldConfig(index_type=NebulaIndexType.EXACT),
    )
...
class EkKloz(BaseNode):
    ...
    node_config: ClassVar[NodeModelConfig] = NodeModelConfig(
        ...
        cardinality=True,
        how_to_extract=HowToExtract.CASE_0,
        nodeclass_to_be_created_automatically=EkKlozlar,
    )
...
class EkHizmetler(BaseNode):
    ...
    ek_hizmet_var: bool = Field(default=True, description="En az bir ek hizmet varsa True, yoksa zaten yaratilmaz.")


class EkHizmet(BaseNode):
    """Ek hizmet node."""

    node_config: ClassVar[NodeModelConfig] = NodeModelConfig(
        nodetag_index=False,
        description="Ek hizmetler node. Bu hizmetler poliçe için ek koşulları belirtir.",
        cardinality=True,
        how_to_extract=HowToExtract.CASE_0,
        nodeclass_to_be_created_automatically=EkHizmetler,
    )
    ek_hizmet_adi: str = Field(description="Ek hizmet adı")
    ek_hizmet_aciklama: str | None = Field(
        default=None,
        description="Ek hizmet açıklaması. Bu açıklamalar poliçe için ek koşulları belirtir.",
    )
